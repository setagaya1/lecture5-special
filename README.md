...
- __パスパラメーターとクエリ文字列との違いは何か？__
  - 前述のとおり、クエリ文字列はURLに追記することで+αの分析や表示ができるものであるため、**省略可**である。
  - 一方で、パスパラメーターはWebサイトを表示のためサーバーへリクエストを送る際に必要不可欠であるため**省略不可**である。  
（というよりも、厳密には省略可能な場合もあり、省略した際はデフォルト値が補足されるイメージか？index.htmlなど）
## 5. HTTPメソッドとは何か？
